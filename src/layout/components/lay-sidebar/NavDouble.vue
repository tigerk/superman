<script setup lang="ts">
  ...
  const { title, pureApp, getLogo, isCollapse, tooltipEffect, menuSelect, backTopMenu, toggleSideBar } = useNav();
  ...
  <div class="w-[290px]">
    <div :class="['double-left', 'w-[80px]', 'h-full', showLogo ? 'has-logo' : 'no-logo']">
      ...
      <el-scrollbar wrap-class="scrollbar-wrapper">
        <ul>
          <li v-for="(menu, index) in menuData" :key="index" :class="curActive === index ? 'is-active' : ''" @click="handleChildMenu(menu, index)">
    ...
    >
      ...
      <el-scrollbar wrap-class="scrollbar-wrapper">
        ...
        >
          <LaySidebarItem v-for="routes in childMenu" :key="routes.path" :item="routes" :base-path="routes.path" class="outer-most select-none" />
        </el-menu>
      </el-scrollbar>
      <LaySidebarCenterCollapse v-if="isShow || isCollapse" :is-active="pureApp.sidebar.opened" @toggleClick="toggleSideBar" />
